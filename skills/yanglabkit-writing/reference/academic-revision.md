# Academic-revision layer

The conditional layer of `yanglabkit-writing`, for scholarly manuscripts under
reviewer pressure. It extends — never replaces — the core principles in
`prose-principles.md`.

## When this layer activates

Apply it when any of these hold:

- The target is a scholarly manuscript, referee response, or grant narrative.
- The user is revising in response to reviewer/referee comments.
- The task involves the paper's framing: thesis, novelty, contribution, or
  terminology.

For everyday prose (emails, posts, documentation), the core layer alone
applies.
...
still be rejected for not earning its place.
...
- *Tell:* a "conceptual slippage" reviewer complaint; related terms used
  interchangeably for the central claim.
- *Practice:* choose the spine term once; each relative then appears only where
...
contribution) or a *downstream consequence* (too generic, shared with many
papers). The differentiating, falsifiable middle link is the thesis.

- *Practice:* write the causal chain out explicitly, then ask of each link:
...
the visible exceptions as the tip of the iceberg rather than a refutation
(survivorship framing: the cases that surface are the extreme, traceable ones;
the modal case never surfaces).
